...
class Mind {
    constructor({ container = '#mind-map-container', data = {}, options = {} }) {
        ...
        this.options = {
            style: {
                fill: color_1.default.blue,
                line: color_1.default.line,
                font: color_1.default.font,
            },
            lineType: 'polygon',
            legends: null,
        };
        ...
        this.scale = 10;
        if (!dom_1.getDom(container)) {
            throw Error('null canvas container found');
        }
        this.oCon = dom_1.getDom(container);
    ...
    _init() {
        ...
        const oCanvas = dom_1.createDom('canvas');
    ...
    _setData(data) {
        this.data = data;
        // scan the tree
        tree_1.figureNodeLevel(this.data);
    }
    _render() {
        const { data } = this;
        let queue = [];
    ...
    _initToolbar(selector) {
        this.toolbar = new toolbar_1.default({ selector });
    ...
    _initLegends() {
        ...
        const oBox = dom_1.createDom('div', 'legends-container');
        let tpl = ``;
